...
def buttonChooseClick():
 ...
 \
        ...
        lb.place(x=50, y=130, width=260, height=240)
    if not folder:
        return
    global playing
    ...
    if len(res):
        ...
        while playing:
            if not pygame.mixer.music.get_busy():
                ...
                NumberSong = num + 1
                print("当前为第{}首歌".format(NumberSong))
                ...
                MusicTimeInfoMin = MusicTime//60
                MusicTimeInfoSec = MusicTime%60
                MusicTimeInfo = (str(MusicTimeInfoMin),':',str(MusicTimeInfoSec))
                MusicTimeInfoN = ''.join(MusicTimeInfo)
                print(u"音乐时长为: <{}:{}>".format(MusicTime//60,MusicTime%60))
                
                # 歌曲信息显示
                
                Musicinfo = (NewnextMusic,'\n当前为第: ',str(NumberSong),'首歌',\
                           '\n歌曲时长为: ',MusicTimeInfoN)
                musicName.set('当前歌曲: ' + ''.join(Musicinfo))
...
labelName.place(x=20, y=400, width=450, height=100)
